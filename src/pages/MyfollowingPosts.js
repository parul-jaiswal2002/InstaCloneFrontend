...
const MyfollowingPosts = () => {
  ...
  const [commSectionItems, setCommSectionItems] = useState([])
  ...
  //Function for toggling comment section
  const toggleComment = (post) => {
    if(show){
      ...
      setShow(true)
      setCommSectionItems(post)
      console.log(commSectionItems)
    }
  ...
  useEffect(() => {
    ...
    fetch('https://instaclonebackend-5yyd.onrender.com/post/myfollowingPosts', {
      headers : {
  ...
  const likePost = (id) => {
    fetch('https://instaclonebackend-5yyd.onrender.com/post/like', {
      method : 'put',
      headers : {
    ...
    .then(result => {
      //we got new post as we updated the last one that's way here we are also updating setPosts
  ...
  const unlikePost = (id) => {
    fetch('https://instaclonebackend-5yyd.onrender.com/post/unlike', {
      method : 'put',
      headers : {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer ' + localStorage.getItem('jwt')
  ...
  const makeComment = (comment, id) => {
    fetch('https://instaclonebackend-5yyd.onrender.com/post/comments', {
      method : 'put',
      headers : {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer ' + localStorage.getItem('jwt')
          ...
          <div className="card" key={post._id}>
              ...
              <div className="card-header">
                <div className="profile-pic">
                    <img src={post.postedBy.profilepic ? post.postedBy.profilepic : defaultProfilePic} alt="profile-pic" />
                </div>
                <Link to={`/profile/${post.postedBy._id}`}><h5>{post.postedBy.name}</h5></Link>
              </div>
              {/* card-post */}
              ...
              <div className="card-content">
                  ...
                  <p>{post.body}</p>
                  <p style={{color: 'grey', cursor:'pointer'}} onClick={() => toggleComment(post)}>view all comments</p>
              ...
              <div className="add-comment">
                ...
                <button className='post' onClick={() => makeComment(comment, post._id)}>Post</button>
              </div>
          </div>
           )
        })}
          ...
          <div className="postDetails">
            {/* card-header */}
              ...
              <div className="card-content">
                  ...
                  <p>{commSectionItems.likes.length} likes</p>
                  <p>{commSectionItems.body}</p>
              ...
              <div className="add-comment">
                ...
                <button className='post'
                 onClick={() =>{
                  makeComment(comment, commSectionItems._id)
                  toggleComment()
                 }}>Post</button>
              </div>
          </div>
